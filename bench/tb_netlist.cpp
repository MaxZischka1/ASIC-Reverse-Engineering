// tb_netlist.cpp — replay the recorded waveform against the flat netlist.
//
// DUT: netlistVerilog (bench/netlistVerilog.v). Same replay bench as
// tb_cones.cpp on the other DUT: inputs I/enable/rst_n each cycle come from
// the stimulus block inlined below (bench/vcdToStimulus.py on the recording);
// the recorded O and success are the expected outputs. Reports the first cycle
// the DUT diverges.
//
//   make -C bench netlist                      summary line only
//   ./obj_dir_netlist/VnetlistVerilog +verbose per-cycle detail
//
...
#define STIM_IX_I 3
#define STIM_W_I 1
static const char* const STIM_I =
  "000000101010000000101100001010011000000000100000011101100001001011000011"
  "100110000000101100000010111000000000100000110011100000000000000000000000"
...
class TxDrive {
    VnetlistVerilog* dut;
public:
    TxDrive(VnetlistVerilog* dut) : dut(dut) {}
    void drive(TxIn* tx) {
        dut->I      = tx->I;
...
// ------------------------------------------------------------------- monitors
class MonIn {
    VnetlistVerilog* dut; Scb* scb; int c = 0;
public:
    MonIn(VnetlistVerilog* dut, Scb* scb) : dut(dut), scb(scb) {}
    void monitor() {
        TxIn* tx = new TxIn();
        tx->cycle  = c++;
        tx->I      = dut->I;
...
class MonOut {
    VnetlistVerilog* dut; Scb* scb;
public:
    MonOut(VnetlistVerilog* dut, Scb* scb) : dut(dut), scb(scb) {}
    void monitor() {
        TxOut* tx = new TxOut();
        tx->O       = (uint64_t)dut->O;
...
int main(int argc, char** argv) {
    ...
    VnetlistVerilog* dut = new VnetlistVerilog();
    VerilatedVcdC*   tfp = new VerilatedVcdC();
    setup(dut, tfp, argc, argv, "waveform_netlist.vcd");
