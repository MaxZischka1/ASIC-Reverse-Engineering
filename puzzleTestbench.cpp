// puzzleTestbench.cpp -- drive the reverse-engineered puzzle netlist
// (puzzleNetlist.v, generated by genVerilog.py) and check it against the
// reference waveform shipped with the puzzle.
//
// PORTS
// =====
// The DUT's real ports, per MODULE_GRAPH.json and confirmed against
// example_inputs.vcd's own $var list:
//     inputs : clk, rst_n, enable, I      outputs: O[7:0], success
// This file used to be a copy of the warm-up adder bench and drove
// A/B/en/S, none of which this design has.
//
// PROTOCOL (recovered from example_inputs.vcd, not guessed)
// =========================================================
// example_inputs.vcd contains two back-to-back frames with an identical
// shape, which is what defines a "frame" here:
//     3 cycles  rst_n=0, enable=0     -- reset
//     1 cycle   rst_n=1, enable=0     -- idle
//   121 cycles  rst_n=1, enable=1     -- shift 121 bits in on I, in order
//    35 cycles  rst_n=1, enable=0     -- readout
// During readout the design drives ASCII codes on O, one character per
// cycle, with O=0 on the cycles between characters. That is the "output
// generator" the puzzle README says to ignore while reverse-engineering
// but to simulate for the final answer. Concatenating the non-zero O
// bytes over the readout window yields the message.
//
// SCOREBOARD: REAL GOLDEN DATA, NOT A GUESSED MODEL
// =================================================
// We do not know the design's function -- recovering it is the puzzle --
// so there is no reference model to compare against, and inventing one
// would only test our guess against itself. But example_inputs.vcd gives
// both the stimulus AND the response the real silicon produced, so the
// two frames below are a genuine end-to-end regression test of the whole
// extraction chain (GDS -> nets -> cells -> netlist): feed the recorded
// 121 input bits, and the netlist must reproduce the recorded message
// ("TRY AGAIN") and must hold success low. If those match, the netlist
// behaves like the real design on the only inputs we have ground truth
// for. The bits below are transcribed from that file.
//
// USAGE
// =====
//   ./Vpuzzle                 replay both reference frames and check them
//   ./Vpuzzle <121 bits>      drive one custom frame, print O and success
//                             (a '0'/'1' string, in shift order)
// Either way a VCD lands in waveform_puzzle.vcd.
...
static const RefFrame REFERENCE_FRAMES[] = {
